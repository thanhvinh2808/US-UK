...
const MAX_MISTAKES_STORED = 500; // giới hạn để tránh phình to localStorage vô hạn
...
export const storage = {
  ...
  // ==========================================================================
  // NGÂN HÀNG CÂU SAI (Mistake Bank) — lưu lại mọi câu người dùng làm sai ở
  // bất kỳ module luyện tập nào (Ngữ pháp, Phát âm, Minimal Pairs, Flashcards,
  // Shadowing...) để họ có thể quay lại ôn riêng đúng những gì mình còn yếu,
  // và để Dashboard tổng hợp thống kê điểm yếu theo từng kỹ năng.
  // ==========================================================================
   ...
   * @param {string} [mistake.topicId]       - id bài học liên quan (nếu có), để có thể nhảy thẳng tới ôn lại
   */
  saveMistake: (mistake) => {
    try {
   ...
   * sắp xếp giảm dần — kỹ năng nào sai nhiều nhất đứng đầu. Dùng để hiển thị
   * biểu đồ/ cảnh báo "bạn đang yếu nhất ở đâu" trên Dashboard.
   */
  getWeaknessStats: () => {
    try {
      const list = storage.getMistakes();
